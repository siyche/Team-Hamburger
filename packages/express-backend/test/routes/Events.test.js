...
describe('Events Routes (POST and GET /events/events)', () => {
  ...
  // fakeCalendar must be in scope for beforeEach and tests
  const fakeCalendar = { _id: 'cal1', events: [], save: calendarSaveMock };

  beforeEach(() => {
    jest.clearAllMocks();
    // Stub Calendar.findById to return the fakeCalendar subdoc
    Calendar.findById.mockResolvedValue(fakeCalendar);
  });
  ...
  describe('GET /events/events', () => {
    it('returns events array with 200 when found', async () => {
        ...
        User.findOne.mockResolvedValue({ calendars: ['cal1'] });
        const fakeCalWithEvents = { _id: 'cal1', events: fakeEvents };
        Calendar.findById.mockResolvedValue(fakeCalWithEvents);

        const res = await request(app).get('/events/events');

        expect(User.findOne).toHaveBeenCalledWith({ email: 'test@example.com' });
        expect(Calendar.findById).toHaveBeenCalledWith('cal1');
        expect(res.status).toBe(200);
        expect(res.body).toEqual(fakeEvents);
    });

    it('returns 401 if no user on GET', async () => {
